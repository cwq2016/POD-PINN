...
import torch
import os
from Cases_test import gen_testcases, NumSolsdir, M_Vec, FND, Ncut, useClosure
...
#resultsdir = 'results'
# validation space
file_Validation = NumSolsdir + '/' + 'Snapshots_Validation.npz'
...
def CalculateError(testcases):
    ...
    for case in testcases:
        ...
        filePOD_PODG = filePOD.replace('Snapshots_POD_SVD', PODG_prefix+'Snapshots_POD_SVD')
        ...
        lamda_G_PODG= None
        if os.path.isfile(PODG_Projection_file):
        ...
        # PODG error
        Error[ind,indM,0] = roeqs.GetError(lamda_Projection,   Snapshots_V)
        Error[ind,indM,3] = roeqs.GetError(lamda_G,            Snapshots_V)
        Error[ind,indM,4] = roeqs.GetError(lamda_G_useclosure, Snapshots_V)        
        Ntcut = PODG_alpha_p.shape[1] - alpha_p.shape[1]
        ...
        lamda_Net = Net(torch.tensor(alpha.reshape((-1,3))).float().to(DEVICE)).reshape((Np,Nt,-1))

        # POD_NN
        Nettype = case['Nettype']
        if Nettype == 'Label':
            Error[ind,indM,1] = roeqs.GetError(lamda_Net, Snapshots_V)
        # POD-Hybrid
        elif Nettype == 'Hybrid':
            #continue
            Error[ind,indM,2] = roeqs.GetError(lamda_Net, Snapshots_V)             
    return Error
#字符|类型 | 字符|类型
#---|--- | --- | ---
#`  '-'	`| 实线 | `'--'`|	虚线
